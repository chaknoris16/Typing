...
public:
    ...
    QList<QVariantMap> getAllResults() const override
    {
        ...
        while (query.next()) {
            QVariantMap result;
            result["Exercise"] = query.value("Exercise Name").toString();
            result["Typing speed"] = query.value("Typing Speed").toInt();
            result["Number of mistakes"] = query.value("Number of Mistakes").toInt();
            result["Datetime"] = query.value("Date Time").toDateTime();
